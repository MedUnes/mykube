...
# Always stop etcd and wipe /var/lib/etcd before installing fresh certs.
# Rerunning certs.sh generates a new CA — etcd must be restarted with the
# new certs and a clean data dir, otherwise it rejects connections with
...
# Stop etcd so we can safely replace certs and wipe stale cluster state
sudo systemctl stop etcd 2>/dev/null || true

# Wipe data dir — stale state from a previous CA causes immediate rejection
sudo rm -rf /var/lib/etcd
sudo mkdir -p /etc/etcd /var/lib/etcd
sudo chmod 700 /var/lib/etcd

# Force-copy fresh certs from ~/ (just distributed by certs.sh)
sudo cp ~/ca.pem       /etc/etcd/ca.pem
sudo cp ~/etcd.pem     /etc/etcd/etcd.pem
sudo cp ~/etcd-key.pem /etc/etcd/etcd-key.pem
