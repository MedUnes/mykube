...
BRIDGE_IP=$(ip addr show  | grep virbr | head -2 | grep 'inet' | awk '{print $2}' | cut -d / -f1)
...
# All three nodes get a kubelet identity cert — including control-plane-1.
#
# WHY control-plane-1 NEEDS A KUBELET CERT:
# Phase 07-nodes installs kubelet on control-plane-1 so that the Cilium
# DaemonSet schedules there. Without Cilium on control-plane-1, no eBPF
# rules are loaded onto the host network interface, and the kube-apiserver
# (a systemd process on the host network) cannot reach ClusterIP services.
# This breaks all admission webhooks with connection timeout errors.
# The kubelet identity cert follows the same pattern as workers:
#   CN=system:node:control-plane-1, O=system:nodes
...
# Every node receives: CA cert + its own kubelet cert/key.
# control-plane-1 additionally receives all control-plane component certs.

distribute_certs() {
  echo ""
  echo "[+] Distributing certificates to nodes..."

  # All three nodes: CA + their own kubelet cert/key
  for i in \
    "$WORKER_1_HOST:$WORKER_1_IP" \
    "$WORKER_2_HOST:$WORKER_2_IP" \
    "$CONTROL_PLANE_HOST:$CONTROL_PLANE_IP"; do
    ...
    echo "[+] Copying kubelet certs to $HOST ($IP)..."
    scp $SSH_OPTS \
      ca.pem \
      "${HOST}.pem" \
      "${HOST}-key.pem" \
  ...
  # Control plane additionally needs all control-plane component certs
  echo "[+] Copying control-plane component certs to $CONTROL_PLANE_HOST..."
  scp $SSH_OPTS \
    ca-key.pem \
...
print_summary() {
  ...
  echo ""
  echo " Next step: Generate kubeconfigs (03-kubeconfig)"
  echo "=============================================="
}
